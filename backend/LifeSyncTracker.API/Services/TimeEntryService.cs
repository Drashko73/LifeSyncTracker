...
public class TimeEntryService : ITimeEntryService
{
    ...
    /// <inheritdoc />
    public async Task<byte[]?> GetEmployerReportPdfAsync(int userId, int projectId, int year, int month)
    {
        var project = await _context.Projects
            .FirstOrDefaultAsync(p => p.Id == projectId && p.UserId == userId);
        ...
        // Group entries by date and calculate rounded hours
        var dailyHours = entries
            .GroupBy(e => e.StartTime.Date)
            .Select(g => new
            {
                Date = g.Key,
                Hours = RoundToHalfHour(g.Sum(e => e.DurationMinutes ?? 0))
            })
            .OrderBy(x => x.Date)
            .ToList();

        var totalHours = dailyHours.Sum(x => x.Hours);

        // Set QuestPDF license (Community license for open-source/personal projects)
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header()
                    .Column(column =>
                    {
                        column.Item().AlignCenter().Text($"Working Hours Report").FontSize(20).Bold();
                        column.Item().AlignCenter().Text($"Project: {project.Name}").FontSize(14);
                        column.Item().AlignCenter().Text($"Period: {periodStart:MMMM yyyy}").FontSize(12);
                ...
                page.Content()
                    ...
                    .Column(column =>
                    {
                        column.Item().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Darken2)
                                    .Padding(5).AlignLeft().Text("Date").Bold();
                                header.Cell().BorderBottom(1).BorderColor(Colors.Grey.Darken2)
                                    .Padding(5).AlignCenter().Text("Hours").Bold();
                            });
                            ...
                            foreach (var day in dailyHours)
                            {
                                table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2)
                                    .Padding(5).Text(day.Date.ToString("yyyy-MM-dd"));
                                table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2)
                                    .Padding(5).AlignCenter().Text($"{day.Hours:0.0}h");
                            }

                            // Total row
                            table.Cell().BorderTop(2).BorderColor(Colors.Grey.Darken2)
                                .Padding(5).Text("Total").Bold();
                            table.Cell().BorderTop(2).BorderColor(Colors.Grey.Darken2)
                                .Padding(5).AlignCenter().Text($"{totalHours:0.0}h").Bold();
                        });
                    });

                page.Footer()
                    .AlignRight()
                    .Text("Generated by LifeSyncTracker")
                    .FontSize(10)
                    .Italic()
    ...
    /// <summary>
    /// Rounds minutes to the nearest half-hour according to the rules:
    /// Less than 30 mins -> 0.5h, 30+ mins -> round to next hour.
    /// E.g., 0-29 mins = 0.5h, 30-59 mins = 1h, 60-89 mins = 1.5h, 90-119 mins = 2h
    /// </summary>
